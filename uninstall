...
@is_mac = !RUBY_PLATFORM[/darwin/].nil?
@homebrew_default_path = @is_mac ? "/usr/local" : "/home/linuxbrew/.linuxbrew"
...
OptionParser.new do |opts|
  ...
  opts.on("-pPATH", "--path=PATH", "Sets Homebrew prefix. Defaults to #{@homebrew_default_path}.") do |p|
...
if homebrew_prefix_candidates.empty? # Attempt to locate Homebrew unless `--path` is passed
  ...
  homebrew_prefix_candidates << Pathname.new(@homebrew_default_path) # Homebrew default path
...
HOMEBREW_PREFIX = homebrew_prefix_candidates.find do |p|
  ...
  if p.to_s == @homebrew_default_path && File.exist?("#{@homebrew_default_path}/Homebrew/.git")
    next true
  end
...
if paths.any?
  args = if @is_mac
    # BSD find
    %w[-E] + paths + %w[-regex .*/info/([^.][^/]*\.info|dir)]
  else
    # GNU find
    paths + %w[-regextype posix-extended -regex .*/info/([^.][^/]*\.info|dir)]
  end
  if options[:dry_run]
    args << "-print"
  else
...
if options[:dry_run]
  ...
  if HOMEBREW_PREFIX.to_s != @homebrew_default_path && HOMEBREW_PREFIX.exist?
